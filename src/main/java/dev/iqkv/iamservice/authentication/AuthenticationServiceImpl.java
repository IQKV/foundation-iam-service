...
package dev.iqkv.iamservice.authentication;
...
import dev.iqkv.iamservice.authentication.dto.AuthenticationDtos;
import dev.iqkv.iamservice.denylist.TokenDenylistService;
import dev.iqkv.iamservice.email.EmailVerificationToken;
import dev.iqkv.iamservice.email.EmailVerificationTokenMapper;
import dev.iqkv.iamservice.infrastructure.config.NotificationConfigurationProperties;
import dev.iqkv.iamservice.infrastructure.messaging.MessagingService;
import dev.iqkv.iamservice.infrastructure.messaging.NotificationEvent;
import dev.iqkv.iamservice.infrastructure.messaging.NotificationEventType;
import dev.iqkv.iamservice.lockout.AccountLockoutManager;
import dev.iqkv.iamservice.membership.MembershipService;
import dev.iqkv.iamservice.membership.TenantMembership;
import dev.iqkv.iamservice.membership.TenantMembershipMapper;
import dev.iqkv.iamservice.security.JwtClaimNames;
import dev.iqkv.iamservice.shared.exception.InvalidVerificationTokenException;
import dev.iqkv.iamservice.shared.exception.TenantContextMismatchException;
import dev.iqkv.iamservice.shared.exception.TenantNotAvailableException;
import dev.iqkv.iamservice.shared.exception.TenantSuspendedException;
import dev.iqkv.iamservice.shared.exception.UserNotFoundException;
import dev.iqkv.iamservice.shared.exception.VerificationRateLimitException;
import dev.iqkv.iamservice.tenant.Tenant;
import dev.iqkv.iamservice.tenant.TenantMapper;
import dev.iqkv.iamservice.tenant.TenantStatus;
import dev.iqkv.iamservice.tenancy.TenantContext;
import dev.iqkv.iamservice.user.UserMapper;
...
public class AuthenticationServiceImpl implements AuthenticationService {
  ...
  public AuthenticationDtos.TokenResponse refresh(final AuthenticationDtos.RefreshTokenRequest request) {
    ...
    if (!JwtClaimNames.TYPE_REFRESH.equals(type)) {
      throw new dev.iqkv.iamservice.shared.exception.InvalidTokenTypeException();
    }
  ...
  public List<AuthenticationDtos.TenantMembershipSummary> listUserTenants(
    ...
    for (final TenantMembership membership : memberships) {
      if (membership.getStatus() != dev.iqkv.iamservice.membership.MembershipStatus.ACTIVE) {
        continue;
      }
